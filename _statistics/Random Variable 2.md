...
### **A Deep Dive into Random Variables, Post 2: Understanding Discrete Random Variables**

Welcome back\! In our [first post](https://www.google.com/search?q=random+variables+explained), we established that a random variable is a rule that assigns a number to an outcome. We also introduced the two main types: discrete and continuous. Today, we're zeroing in on **discrete random variables**—the ones that involve counting.
...
1. The probability at any specific value xi​ must be positive: p(xi​)\>0.  
...
* The PMF: The probability of getting exactly k successes in n trials is:  
  P(X=k)=(kn​)pk(1−p)n−k  
  * The (kn​) part (read "n choose k") counts how many different ways you can arrange the *k* successes among the *n* trials.  
  * The pk(1−p)n−k part is the probability of any *one* of those specific arrangements occurring.  
...
* **Example:** You're rolling a die until you get a 6\. The probability of success is p=1/6. The probability that it takes you exactly 3 rolls is P(X=3)=(5/6)2×(1/6).
