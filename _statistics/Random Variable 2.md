...
* **Example:** You flip a fair coin 10 times (n=10,p=0.5). What's the probability of getting exactly 7 heads $(k=7)$?

  $P(K=7)={10 \choose 7}p^7(1−p)^{3}=0.1172$
  
  So, there's about 11.72% of chance of getting exactly 7 heads.
...
* **Example:** You're rolling a die until you get a 6\. The probability of success is $p=1/6$. The probability that it takes you exactly 3 rolls is $P(X=3)=(\frac{5}{6})^2 \times (\frac{1}{6})\approx0.1157$.

  So there's about 11.57% of chance it takes you exactly 3 rolls to get a 6.
...
* **Example:** A call center receives an average of 10 calls per hour (λ=10). The Poisson distribution can tell you the probability of receiving exactly 15 calls in a given hour. It's also famously used to approximate the Binomial distribution when *n* is very large and *p* is very small.
  ...
  $P(X=15)=\frac{e^{-10} \cdot 10^15}{15!}\approx0.0347$

  So there's about 3.47% chance of receiving exactly 15 calls in a given hour.
...
To get a better feel for how these distributions behave, try playing with the parameters in the interactive visualizer below. You can see how the shape (and skewness) of the PMF changes as you adjust the probability of success (p), the number of trials (n), or the average rate (λ).
