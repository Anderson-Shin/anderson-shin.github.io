---
title: "A Deep Dive into Random Variables (Part 1): The Foundation"
collection: statistics
permalink: /statistics/random-variables-part1/
...
![discrete plot](/images/random_variable_1_discrete.png)
...
![continuous plot](/images/radom_variable_1_continous.png)

Here, the top graph is the **Probability Density Function (PDF)**. Unlike the PMF, the value of the PDF is not a probability itself. Instead, the **area under the PDF curve** gives us the probability. The bottom graph, the CDF, directly shows this accumulated area. As the plot perfectly illustrates, the shaded area under the PDF curve up to $x=−0.5$ is $0.31$. This is precisely the value of the CDF at $x=−0.5$, as shown by the dashed line in the bottom graph. So, $F_x​(−0.5)=P(X≤−0.5)=0.31$.

The CDF has three key properties expressed in more formal terms:

1. **It's non-decreasing.** For any $a<b$, we have $F_x​(a)≤F_x​(b)$.  
2. **The lower limit is 0\.** $\text{lim}_{b→−∞}​F_x​(b)=0$.  
3. **The upper limit is 1\.** $\text{lim}_{b→∞​}F_x​(b)=1$.

It's also important to note the subtle difference between P(X≤b) and P(X\<b). For a continuous variable, they are the same because the probability of hitting any single exact point is zero. For a discrete variable, they can be different. We can find the latter using a limit: $P(X<b)=\text{lim}_{h→0}+​F_x​(b−h)$.
